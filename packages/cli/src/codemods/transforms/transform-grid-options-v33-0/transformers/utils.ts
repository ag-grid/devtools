import j, { JSXAttribute, ObjectProperty, ASTPath, Node, Expression } from 'jscodeshift';
...
export const getValueFromNode = (node: ObjectProperty | JSXAttribute): any => {
  if (!node) {
...
export const wrapValue = (path: ASTPath<JSXAttribute | ObjectProperty>, node: any) => {
  if (isJSXAttribute(path)) {
    ...
    return j.jsxExpressionContainer(node as any);
  }
  return node;
};
...
export const getSibling = <T extends JSXAttribute | ObjectProperty>(
  ...
  if (path.node.type === 'JSXAttribute') {
    return getSiblings(path).find(
      (child: any) => child && child.name.type === 'JSXIdentifier' && child.name.name === name,
    );
  }
  return getSiblings(path).find(
    (child: any) => child && child.key.type === 'Identifier' && child.key.name === name,
  );
};

export const createTernary = (condition: any, consequent: any, alternate: any) => {
  if (!condition) {
    return consequent;
  }
...
export const createLogicalOr = (left: any, right: any) => {
...
export const createLogicalAnd = (left: any, right: any) => {
